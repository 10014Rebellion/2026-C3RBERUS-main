...
public class Intake {
    ...
    // PIVOT COMMANDS //
    public Command setRackStateCmd(IntakeRackStates pIntakePivotState) {
        return mIntakeRackSS.setStateCmd(pIntakePivotState);
    }

    public Command trashCompactPivotRepeat() {
        return new RepeatCommand(
            new SequentialCommandGroup(
                mIntakeRackSS.setStateCmd(IntakeRackStates.COMPACT_HIGH).withTimeout(tIntakeCompactTime),
                mIntakeRackSS.setStateCmd(IntakeRackStates.COMPACT_LOW).withTimeout(tIntakeCompactTime)
            )
        );
    }

    public Command trashCompactRackContinuous(){
        return mIntakeRackSS.setStateCmd(IntakeRackStates.COMPACT_AMPS);
    }
}
